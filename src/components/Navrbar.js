...
function Navrbar() {
  ...
  return (
    <div className='flex justify-between items-center w-full h-20 text-white fixed bg-black px-4'>
      <div>
        <h1 className='text-5xl font-signature ml-2'>Jani Syed</h1>
      </div>
      ...
      {nav && (
        ...
        {links.map(({id,link})=>(
          <li key={id} className='px-4 cursor-pointer capitalize hover:scale-105 duration-200 text-3xl py-5'>{link}</li>
        ))}
        </ul>
      )}
